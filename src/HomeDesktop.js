...
const HomeDesktop = ({ onStartClick }) => {
  useEffect(() => {
    ...
    // Smooth tilt with requestAnimationFrame and inertia
    const cards = Array.from(document.querySelectorAll('.carousel-card'));
    const state = new Map();
    function rafLoop() {
      cards.forEach(card => {
        const s = state.get(card) || { tx: 0, ty: 0, vx: 0, vy: 0 };
        // integrate velocity -> position (simple lerp)
        s.tx += (s.vx - s.tx) * 0.15;
        s.ty += (s.vy - s.ty) * 0.15;
        const rotX = s.ty * 10; // stronger on root y
        const rotY = s.tx * -14;
  ...
  // Carousel controls: autoplay + arrows + indicators (lightweight)
  const track = document.querySelector('.templates-section .carousel-track');
  const items = Array.from(document.querySelectorAll('.carousel-item'));
  const indicators = Array.from(document.querySelectorAll('.carousel-indicators .indicator'));
  const btnLeft = document.querySelector('.templates-section .carousel-arrow.left');
  const btnRight = document.querySelector('.templates-section .carousel-arrow.right');
  let current = 0;
  let autoplayId = null;
  function setActive(idx) {
    ...
    // visually center by adjusting transform on track
    const offset = (current - 1) * 360 * -1; // quick center calc for 3 cards
    if (track) track.style.transform = `translateX(${offset}px)`;
  }
  function next() { setActive(current+1); }
  function prev() { setActive(current-1); }
  function startAutoplay() { autoplayId = setInterval(next, 3800); }
  function stopAutoplay() { if (autoplayId) { clearInterval(autoplayId); autoplayId = null; } }

  if (btnLeft) btnLeft.addEventListener('click', () => { prev(); stopAutoplay(); });
  if (btnRight) btnRight.addEventListener('click', () => { next(); stopAutoplay(); });
  indicators.forEach(ind => ind.addEventListener('click', (e)=>{ setActive(Number(e.currentTarget.dataset.index)); stopAutoplay(); }));
  ...
  if (!mq.matches) startAutoplay();
    ...
    return () => {
      ...
      if (btnLeft) btnLeft.removeEventListener('click', prev);
      if (btnRight) btnRight.removeEventListener('click', next);
      indicators.forEach(ind => ind.removeEventListener('click', ()=>{}));
      stopAutoplay();
      ...
      <section className="templates-section">
        <div className="container templates-inner">
          ...
          <div className="templates-carousel">
            <button className="carousel-arrow left" aria-label="Previous template">‹</button>
            <div className="carousel-track">
              <div className="carousel-card carousel-item reveal-on-scroll active" data-index="0" tabIndex={0}>
              ...
              <div className="carousel-card carousel-item reveal-on-scroll" data-index="1" tabIndex={0}>
                <div className="card-depth"></div>
                <div className="card-sheen"></div>
                <div className="card-inner"><img src={process.env.PUBLIC_URL + '/assets/hero-illustration.svg'} alt="template2"/></div>
              </div>
              <div className="carousel-card carousel-item reveal-on-scroll" data-index="2" tabIndex={0}>
                <div className="card-depth"></div>
                <div className="card-sheen"></div>
                <div className="card-inner"><img src={process.env.PUBLIC_URL + '/assets/hero-illustration.svg'} alt="template3"/></div>
              </div>
            </div>
            <button className="carousel-arrow right" aria-label="Next template">›</button>
            <div className="carousel-indicators" aria-hidden="true">
              <button className="indicator active" data-index="0"></button>
              <button className="indicator" data-index="1"></button>
              <button className="indicator" data-index="2"></button>
            </div>
          </div>
