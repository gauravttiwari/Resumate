  ...
  const handleAIImprove = async () => {
    ...
    try {
      ...
      const nonEmptyExperiences = experiences.filter(
        (exp) => exp.description && exp.title
      );

      if (nonEmptyExperiences.length === 0) {
        setError(
          'Please add descriptions to your work experiences before improving them.'
        );
        setLoading(false);
        return;
      }

      const improvedExperiences = await improveWorkExperience(
        nonEmptyExperiences
      );

      // Merge the improved experiences back with the original list
      const updatedExperiences = experiences.map((exp) => {
        const improved = improvedExperiences.find(
          (imp) => imp.title === exp.title && imp.company === exp.company
        );
        return improved || exp;
      });
  ...
  return (
    <div className="experience-editor">
            ...
            <div className="form-row">
              <div className="form-group full-width">
                ...
                <small className="helper-text">
                  Tip: Use bullet points and strong action verbs. Quantify
      ...
      <div className="editor-footer">
        <button className="add-button" onClick={addExperience} type="button">
      ...
      <div className="ai-explanation">
        ...
        <p className="tip">
          For best results, add basic descriptions first, then let AI enhance
          them.
        </p>
      </div>
    </div>
